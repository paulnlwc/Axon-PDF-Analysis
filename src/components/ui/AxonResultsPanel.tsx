...
interface AxonResultsPanelProps {
  results: SearchResult[];
  isLoading?: boolean;
}

export function AxonResultsPanel({ results, isLoading = false }: AxonResultsPanelProps) {
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm p-6">
      ...
      >
        <div>
          <h2 className="text-lg font-semibold text-black">Relevant Axon Questions</h2>
          <p className="text-sm text-gray-600 mt-1">
            Create your free account at{' '}
            <a 
              href="https://axonlearning.ai" 
              target="_blank" 
      ...
      <div className="space-y-2 mb-4">
        ...
        >
          {copySuccess ? 'Copied to Clipboard!' : 'Copy Question IDs for Axon Search'}
        </button>
        {results.length > 0 && (
          <p className="text-xs text-center text-black">
            Click to copy formatted IDs for Axon search bar
          </p>
        )}
      </div>
            ...
            results.map((result) => (
              <div key={result.id} className="bg-gray-50 p-4 rounded-lg">
                <div className="flex justify-between items-start mb-2">
                  <span className="text-sm font-medium text-black">Question ID: {result.id}</span>
